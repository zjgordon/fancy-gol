...
#### - [x] P0-I-3 · Docker — @claude, started 2026-09-03, finished 2026-09-03 — bind-mount HMR closed 2026-09-03 after the sandbox remounted the worktree into dind
**Depends on:** P0-I-2 · **Files:** `docker/Dockerfile`, `docker/Dockerfile.dev`, `docker/docker-compose.yml`, `docker/docker-compose.dev.yml`, `.dockerignore`, plus (not in the task's own file list, but required to make the production image buildable at all — see notes) `tsconfig.server.json` and additions to `package.json`'s scripts and `vite.config.ts`'s `server` block.
**Implementation notes**
- Multi-stage `docker/Dockerfile`: `deps` (full `npm ci`, cached separately so a source-only change doesn't reinstall) → `build` (`npm run build`, needing the dev-dependency toolchain) → `runtime` (`node:22-alpine`, `npm ci --omit=dev`, only `dist/` copied in). Runs as the base image's built-in non-root `node` user (uid 1000) — no need to create one. `HEALTHCHECK` shells out to `node -e "fetch(...)"` against `/api/health` rather than installing `curl`/`wget`, keeping the runtime stage's OS package surface at zero beyond the base image. `CMD` invokes `node dist/server/index.js` directly (exec form, not `npm start`) — confirmed locally (see below) that wrapping the entry point in `npm start` stops `SIGTERM` from reaching the process cleanly, which would have silently defeated P0-I-2's graceful-shutdown work inside the container.
- **A real production build needs a compiled server, which didn't exist before this task.** `npm run build` was client-only (`vite build`); the server ran only via `tsx` (a devDependency), and shipping `tsx` into the runtime stage to keep using it there would itself be a "No Bloat" violation (its transitive deps, including a native `esbuild` binary, aren't part of the declared `express`+`ws` runtime surface). Added `tsconfig.server.json` (extends the root config, `noEmit: false`, scoped to `src/server/**`) and a `build:server` script (`tsc -p tsconfig.server.json`), folded into `build` (`vite build && npm run build:server`) so `npm run verify`'s existing `build` step now also catches a broken server compile, and a `start` script (`node dist/server/index.js`) for convenience. `src/server/index.ts`'s import of `./app` was changed to the explicit `./app.js` — Node's native ESM loader requires a resolvable extension on relative specifiers, `tsc` emits it verbatim, and TS resolves a `.js` specifier back to a sibling `.ts` source both at typecheck time and under `tsx watch` (confirmed: `tests/integration/server-process.spec.ts`, which spawns the real `tsx` entry point, still passes unchanged).
- `vite.config.ts`'s dev server binds `host: true` (all interfaces) **and** `allowedHosts: true`. `host: true` is required for the published port to reach Vite at all (Vite defaults to localhost-only). `allowedHosts: true` is the Vite 8 counterpart: with only `host: true`, a request whose `Host` header is anything other than `localhost`/`127.0.0.1` is 403'd. Confirmed against the running dev image: `Host: sandbox-dind` → 403 before the change, 200 after. Local `npm run dev` via `localhost` is unaffected.
- `docker-compose.dev.yml` bind-mounts the repo root over `/app` for live HMR, with a named volume layered on top of `/app/node_modules` specifically — the standard Compose pattern that stops the bind mount from shadowing the container's own (Linux-native) installed modules with whatever (or nothing) exists in the host's own `node_modules`.
- Compose files now set an explicit project `name:` (`fancy-gol` / `fancy-gol-dev`) and `image:` tag (`fancy-gol:latest` / `fancy-gol:dev`). Without that, both files live in `docker/` so Compose's default project name is `docker` and both tag `docker-app:latest` — building the dev image silently overwrites the production one. Caught during revalidation.
- **Revalidation 2026-09-03 (Docker daemon now works for build/run):** `DOCKER_HOST=tcp://sandbox-dind:2376` is a working remote engine — `docker compose -f docker/docker-compose.yml build` and `up` succeed, the container reports `healthy`, and the published `:8080` serves the Gosper gun (Playwright against `http://sandbox-dind:8080/`, tick advancing, canvas painted, first frame ~49 ms, no console errors). Published ports land on the dind host, not this workspace's `127.0.0.1`.
- **Bind-mount HMR was blocked, then unblocked, by the sandbox — not by a compose change.** The first revalidation (same day) found `DOCKER_HOST=tcp://sandbox-dind:2376` resolving `../` to `/workspace/github/fancy-gol` on a daemon that did not have that path, so Docker created an empty directory and hid `/app/package.json`. A later sandbox remount made the agent worktree visible to dind at that same absolute path (`docker run -v /workspace/github/fancy-gol:/probe` now sees `package.json`). The compose file was not rewritten.
- **Revalidation 2026-09-03 (bind-mount HMR):** `docker compose -f docker/docker-compose.dev.yml up --build` (fresh named `node_modules` volume) starts Vite in 231 ms and Express on `:8080`. Published ports still land on the dind host (`http://sandbox-dind:5173/`, `http://sandbox-dind:8080/api/health` → `{ ok: true, version: "0.1.0", uptime }`; agent `127.0.0.1:5173` refused). Playwright against Vite: title `fancy-gol`, gun running (tick advancing, population ~47–144), canvas 1280×720, `window.__fancyGolFirstFrameMs` = 39.5 ms, zero console errors/warnings. Editing `src/client/index.html` **on the agent worktree** produced Vite `page reload index.html`; Playwright then showed a new HUD row (`hmr-probe` / `P0-I-3`) with no image rebuild and no manual navigation. Reverting the edit produced a second `page reload` and the probe row disappeared. That is HMR against mounted sources, the interaction the AC names.
**Acceptance criteria**
- [x] `docker compose -f docker/docker-compose.yml up` serves the working app on `:8080` — built and started for real. `GET /api/health` → `{ ok: true, version: "0.1.0", uptime }`; `index.html` `Cache-Control: no-store`; hashed `/assets/*` `immutable`; unknown `/api/*` JSON 404; SPA fallback for `/foo`. Browser: title `fancy-gol`, tick 632→700, population ~148–163, canvas 1280×720 with painted pixels, `window.__fancyGolFirstFrameMs` ≈ 48.6 ms, zero console errors.
- [x] `docker compose -f docker/docker-compose.dev.yml up` gives working HMR against mounted sources — 2026-09-03 after the sandbox remounted `/workspace` into dind. Vite `page reload index.html` on an agent-worktree edit of `src/client/index.html`; Playwright at `http://sandbox-dind:5173/` showed the new HUD label without a rebuild. Named volume still overlays `/app/node_modules`.
