...
#### - [!] P0-I-3 · Docker — @claude, started 2026-09-03, finished 2026-09-03 — blocked on this sandbox's Docker daemon being unable to build or pull any image, for 3 of 3 criteria
...
- `vite.config.ts`'s dev server now binds `host: true` (all interfaces), required for the dev container's published port to reach it — Vite defaults to localhost-only, which Docker's port mapping can't forward into.
...
- **This sandbox's Docker daemon cannot build or pull any image at all** — confirmed systemically, not specific to this task's Dockerfiles: a one-line sanity `FROM alpine:3.20 / RUN echo hello` fails identically, as does a bare `docker pull node:22-alpine` and `docker run hello-world`, all with the same root cause (`failed to mount ... function not implemented` — the nested daemon's overlayfs snapshotter can't perform the mount/unmount syscalls image extraction and every build stage need). Tried both the BuildKit and legacy builders; both fail the same way. This is an environment/infrastructure limitation, not a defect in the Dockerfiles, but it means none of this task's three acceptance criteria could be exercised as written — see below for what *was* verified instead, and for what remains genuinely unproven.
- What was verified without Docker, as the closest available substitute: (1) reproduced the runtime stage's exact `npm ci --omit=dev` in a scratch directory from just `package.json`/`package-lock.json` — 69 packages, 4.6 MB, entirely `express`+`ws`'s own transitive tree, zero dev tooling present; (2) ran the compiled `dist/server/index.js` against that production-only `node_modules` and confirmed `/api/health`, asset caching, the SPA fallback, and the unknown-`/api/*` JSON 404 all behave correctly, then sent it a real `SIGTERM` and confirmed a clean exit in 5 ms; (3) ran `npm run dev` directly (the same command `Dockerfile.dev`'s `CMD` invokes) and confirmed Vite bound to all interfaces (not just localhost) and the Express dev server started alongside it, both cleanly. None of this substitutes for the real ACs — no container was ever built, so the base-image choice, the non-root `USER` directive, the `HEALTHCHECK` instruction, the bind-mount/named-volume interaction, and the final image size are all unverified in a real container, honestly labelled as such rather than assumed.
**Acceptance criteria**
- [!] `docker compose -f docker/docker-compose.yml up` serves the working app on `:8080` — blocked: this sandbox's Docker daemon cannot build any image (systemic; see notes). Substitute verification: the exact production `node_modules` plus the exact compiled `dist/` the image would contain were run directly with plain `node` and serve `:8080` correctly (health, assets, SPA fallback, unknown-API 404) — but the container itself was never built.
- [!] `docker compose -f docker/docker-compose.dev.yml up` gives working HMR against mounted sources — blocked: same Docker limitation. Substitute verification: `Dockerfile.dev`'s `CMD` (`npm run dev`) runs correctly outside the container, with Vite now bound to all interfaces per the `vite.config.ts` change this task made — but the bind-mount/named-volume interaction the compose file relies on was never exercised.
- [!] Production image < 250 MB, runs as UID ≠ 0, and reports healthy — blocked: no image was ever built, so none of these three are measured. The `USER node` directive and `HEALTHCHECK` instruction are believed correct by inspection (a documented, well-known base-image user; the healthcheck command was hand-run successfully against a live server outside Docker) but not confirmed running inside an actual container.
