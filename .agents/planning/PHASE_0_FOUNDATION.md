...
#### - [!] P0-I-3 · Docker — @claude, started 2026-09-03, revalidated 2026-09-03 — production compose, size, UID and health green; blocked on bind-mount HMR because this sandbox's Docker daemon is a remote dind that cannot see the workspace filesystem
...
- **Bind-mount HMR still cannot be exercised in this sandbox.** Compose resolves `../` to `/workspace/github/fancy-gol` on the *client*, then asks the remote daemon to bind-mount that path; the path does not exist on dind, so Docker creates an empty directory and the mount hides the image's `COPY`'d `/app` (`npm` exits `ENOENT` on `/app/package.json`). This is a remote-daemon filesystem isolation limit, not a defect in the compose file. `Dockerfile.dev`'s `CMD` (`npm run dev`) *does* work in a real container when the bind mount is omitted: Vite + Express come up, the gun runs at `:5173`, and editing `/app/src/client/index.html` inside the container triggered Vite's `page reload index.html` — Playwright saw the new HUD label without a manual navigation. That proves in-container HMR, not "against mounted sources."
**Acceptance criteria**
