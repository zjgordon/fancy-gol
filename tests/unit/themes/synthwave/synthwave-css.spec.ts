...
const CSS = readFileSync(join(process.cwd(), 'src/themes/synthwave/synthwave.css'), 'utf8');
...
describe('themes/synthwave/synthwave.css — matches tokens.ts exactly (no drift)', () => {
  it('has one declaration per SYNTHWAVE_TOKENS entry, same values', () => {
    const declared = Object.fromEntries(declaredTokens(CSS));
    const expected = Object.fromEntries(tokenEntries(SYNTHWAVE_TOKENS));
