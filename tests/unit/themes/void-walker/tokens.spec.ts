...
describe('Void-Walker tokens — WCAG AA chrome-text contrast (P3-C-5)', () => {
  it.each(chromeTextPairs(VOID_WALKER_TOKENS.color))('%s clears 4.5:1', (label, fg, bg) => {
    expect(contrastRatio(fg, bg), `${label} contrast ratio`).toBeGreaterThanOrEqual(4.5);
  });
});

describe('Void-Walker tokens — considered against Default, not copied', () => {
  it('does not inherit Default colour, type, radius, shadow, or motion', () => {
    expect(VOID_WALKER_TOKENS.color).not.toEqual(DEFAULT_DARK_TOKENS.color);
    expect(VOID_WALKER_TOKENS.type.fontFamily).not.toBe(DEFAULT_DARK_TOKENS.type.fontFamily);
    expect(VOID_WALKER_TOKENS.type.letterSpacing.wide).not.toBe(DEFAULT_DARK_TOKENS.type.letterSpacing.wide);
    expect(VOID_WALKER_TOKENS.radius).not.toEqual(DEFAULT_DARK_TOKENS.radius);
    expect(VOID_WALKER_TOKENS.shadow).not.toEqual(DEFAULT_DARK_TOKENS.shadow);
    expect(VOID_WALKER_TOKENS.motion.duration).not.toEqual(DEFAULT_DARK_TOKENS.motion.duration);
  });

  it('is wide-tracked translucent glass', () => {
    expect(VOID_WALKER_TOKENS.type.letterSpacing.wide).toBe('0.18em');
    expect(VOID_WALKER_TOKENS.effect.blur.chrome).toMatch(/14px/);
    expect(VOID_WALKER_TOKENS.color.bg).toBe('#05010f');
  });
});
