...
describe('Sids-Place tokens — WCAG AA ink-on-parchment contrast (P3-C-4)', () => {
  it.each(chromeTextPairs(SIDS_PLACE_TOKENS.color))('%s clears 4.5:1', (label, fg, bg) => {
    expect(contrastRatio(fg, bg), `${label} contrast ratio`).toBeGreaterThanOrEqual(4.5);
  });
});

describe('Sids-Place tokens — considered against Default, not copied', () => {
  it('does not inherit Default colour, type, radius, shadow, or motion', () => {
    expect(SIDS_PLACE_TOKENS.color).not.toEqual(DEFAULT_DARK_TOKENS.color);
    expect(SIDS_PLACE_TOKENS.type.fontFamily).not.toBe(DEFAULT_DARK_TOKENS.type.fontFamily);
    expect(SIDS_PLACE_TOKENS.type.fontFamilyMono).not.toBe(DEFAULT_DARK_TOKENS.type.fontFamilyMono);
    expect(SIDS_PLACE_TOKENS.radius).not.toEqual(DEFAULT_DARK_TOKENS.radius);
    expect(SIDS_PLACE_TOKENS.shadow).not.toEqual(DEFAULT_DARK_TOKENS.shadow);
    expect(SIDS_PLACE_TOKENS.motion.duration).not.toEqual(DEFAULT_DARK_TOKENS.motion.duration);
  });

  it('is serif chrome with a sans for data', () => {
    expect(SIDS_PLACE_TOKENS.type.fontFamily).toMatch(/Palatino|Georgia|serif/i);
    expect(SIDS_PLACE_TOKENS.type.fontFamilyMono).toMatch(/sans/i);
  });
});
