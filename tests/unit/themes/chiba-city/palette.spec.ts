...
import {
  ...
  makeChibaPalette,
  type StateRamp,
} from '@themes/chiba-city/palette';
...
const BG = CHIBA_CITY_TOKENS.color.bg;
...
describe('Chiba-City ramp — distinguishable under CVD simulation (P3-C-2)', () => {
  it('has 8 live states', () => {
    expect(CHIBA_STATE_RAMP).toHaveLength(8);
  });

  it.each(KINDS)('every pair of states stays separated under simulated %s', (kind) => {
    const simulated = steadyColors(CHIBA_STATE_RAMP).map((rgb) => simulateColorBlindness(rgb, kind));
  ...
  it("a state's born colour is lighter than its steady colour (white-hot ignition)", () => {
    for (const { born, steady } of CHIBA_STATE_RAMP) {
...
describe('makeChibaPalette', () => {
  const palette = makeChibaPalette(BG);
  ...
  it('state 1 cools from white-hot toward green (the Chiba signature)', () => {
    ...
    expect(steady.g).toBeGreaterThan(steady.r);
    expect(CHIBA_AGE_TABLE).toHaveLength(PALETTE_STATE_COUNT);
  });

  it('live cells stay readable against the canvas from cellSize 0.5 to 64', () => {
    // Colour contrast is independent of zoom; cellSize 0.5–64 is the claimed readable range.
