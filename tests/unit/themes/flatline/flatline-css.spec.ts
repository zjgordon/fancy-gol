...
const CSS = readFileSync(join(process.cwd(), 'src/themes/flatline/flatline.css'), 'utf8');
...
describe('themes/flatline/flatline.css — matches tokens.ts exactly (no drift)', () => {
  it('has one declaration per FLATLINE_TOKENS entry, same values', () => {
    const declared = Object.fromEntries(declaredTokens(CSS));
    const expected = Object.fromEntries(tokenEntries(FLATLINE_TOKENS));
